...
function StandardLayout({ page, resolveHref }: { page: PageContent; resolveHref: (href?: string) => string }) {
  if (page.slug === "all-access") {
    return <AllAccessLayout page={page} resolveHref={resolveHref} />;
  }
  if (page.slug === "community") {
    return <CommunityLayout page={page} resolveHref={resolveHref} />;
  }
  if (page.slug === "about-us") {
    return <AboutLayout page={page} resolveHref={resolveHref} />;
  }
  return (
    <>
...
function AllAccessLayout({ page, resolveHref }: { page: PageContent; resolveHref: (href?: string) => string }) {
  const sections = page.sections || [];
  const perks = page.bullets || [];
  return (
    <>
      <section className="relative overflow-hidden bg-[#f3efe6] py-20">
        <div className="absolute inset-0 -z-10">
          <div className="absolute left-0 top-0 h-[340px] w-[340px] rounded-full bg-primary/12 blur-[160px]" />
          <div className="absolute right-10 top-10 h-[260px] w-[260px] rounded-full bg-secondary/18 blur-[140px]" />
        </div>
        <div className="mx-auto grid w-full max-w-6xl gap-10 px-6 md:grid-cols-[1.1fr_0.9fr]">
          <div className="space-y-6">
            ...
            <h1 className="font-display text-4xl md:text-6xl">{page.title}</h1>
            {page.subtitle ? <p className="text-lg text-slate-600">{page.subtitle}</p> : null}
            {page.description ? <p className="text-sm text-slate-600">{page.description}</p> : null}
            {page.cta ? (
              <a
                href={resolveHref(page.cta.href)}
                className="inline-flex items-center gap-2 rounded-full bg-primary px-6 py-3 text-xs font-semibold uppercase tracking-[0.2em] text-white"
              >
                {page.cta.label}
                <span>→</span>
              </a>
          ...
          <div className="relative">
            <div className="absolute -inset-4 -z-10 rounded-[32px] bg-white/70 blur-2xl" />
            {page.heroImage ? (
              <div className="overflow-hidden rounded-[26px] border border-slate-200/70 bg-white shadow-[0_24px_60px_rgba(15,23,42,0.12)]">
                <img src={page.heroImage} alt={page.title} className="h-[360px] w-full object-cover" />
              </div>
            ) : (
              <div className="h-[360px] rounded-[26px] border border-dashed border-slate-200 bg-white" />
            )}
          </div>
        </div>
      </section>

      <Section className="bg-white border-t border-slate-200/70" pad="2.6rem">
        <div className="grid gap-8 md:grid-cols-[0.9fr_1.1fr]">
          <div className="space-y-4">
            <p className="text-xs font-semibold uppercase tracking-[0.3em] text-slate-400">Membership arc</p>
            <h2 className="font-display text-3xl md:text-4xl">What you unlock</h2>
            <p className="text-sm text-slate-600">
              A complete learning track with feedback loops built in. You get library access, critique, and a community
      ...
      {perks.length ? (
        <Section className="bg-[#fbfaf7] border-t border-slate-200/70" pad="2.6rem">
          <div className="grid gap-8 md:grid-cols-3">
            {perks.map((perk, index) => (
              <div key={perk} className="space-y-3">
                <div className="text-xs font-semibold uppercase tracking-[0.3em] text-slate-400">
                  Benefit {String(index + 1).padStart(2, "0")}
                </div>
                <p className="text-lg font-semibold text-slate-900">{perk}</p>
              </div>
            ))}
          </div>
        </Section>
...
function CommunityLayout({ page, resolveHref }: { page: PageContent; resolveHref: (href?: string) => string }) {
  const sections = page.sections || [];
  return (
    <>
      <section className="relative overflow-hidden bg-slate-950 py-20 text-white">
        <div className="absolute inset-0 -z-10 bg-[radial-gradient(circle_at_top_left,_rgba(13,110,106,0.25),_transparent_55%),radial-gradient(circle_at_bottom_right,_rgba(214,153,67,0.18),_transparent_60%)]" />
        <div className="mx-auto grid w-full max-w-6xl gap-10 px-6 md:grid-cols-[1fr_1fr]">
          <div className="space-y-6">
            {page.eyebrow ? (
              <p className="text-xs font-semibold uppercase tracking-[0.35em] text-white/60">{page.eyebrow}</p>
            ) : null}
            <h1 className="font-display text-4xl md:text-6xl">{page.title}</h1>
            {page.subtitle ? <p className="text-lg text-white/70">{page.subtitle}</p> : null}
            {page.description ? <p className="text-sm text-white/60">{page.description}</p> : null}
            {page.cta ? (
              <a
                href={resolveHref(page.cta.href)}
                className="inline-flex items-center gap-2 rounded-full bg-white/10 px-6 py-3 text-xs font-semibold uppercase tracking-[0.2em] text-white"
              >
                {page.cta.label}
                <span>→</span>
              </a>
          ...
          <div className="grid gap-4">
            {sections.slice(0, 2).map((section) => (
              <div key={section.heading} className="rounded-[18px] border border-white/10 bg-white/5 p-5">
                <p className="text-sm font-semibold">{section.heading}</p>
                <p className="text-xs text-white/60">{section.body}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {sections.length ? (
        <Section className="bg-white border-t border-slate-200/70" pad="2.6rem">
          <div className="space-y-8">
            {sections.map((section, index) => (
              <div key={section.heading} className="grid gap-6 md:grid-cols-[120px_1fr]">
                <div className="text-xs font-semibold uppercase tracking-[0.3em] text-slate-400">
                  {String(index + 1).padStart(2, "0")}
                </div>
                <div className="grid gap-6 md:grid-cols-[240px_1fr]">
                  <div className="overflow-hidden rounded-[16px] border border-slate-200 bg-slate-100">
                    {section.image ? (
                      <img src={section.image} alt={section.heading} className="h-36 w-full object-cover" />
                    ) : (
...
function AboutLayout({ page, resolveHref }: { page: PageContent; resolveHref: (href?: string) => string }) {
  const sections = page.sections || [];
  return (
    <>
      <section className="relative overflow-hidden bg-white py-20">
        <div className="absolute inset-0 -z-10">
          <div className="absolute right-0 top-0 h-[320px] w-[320px] rounded-full bg-secondary/18 blur-[150px]" />
          <div className="absolute left-20 top-20 h-[240px] w-[240px] rounded-full bg-primary/12 blur-[130px]" />
        </div>
        <div className="mx-auto grid w-full max-w-6xl gap-10 px-6 md:grid-cols-[1fr_1fr]">
          <div className="space-y-6">
            {page.eyebrow ? (
            ...
            {page.cta ? (
              <a
                href={resolveHref(page.cta.href)}
                className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-6 py-3 text-xs font-semibold uppercase tracking-[0.2em] text-slate-700"
              >
                {page.cta.label}
                <span>→</span>
              </a>
          ...
          <div className="relative">
            <div className="absolute -inset-6 -z-10 rounded-[30px] bg-[#f7f4ee] blur-2xl" />
            {page.heroImage ? (
              <div className="overflow-hidden rounded-[26px] border border-slate-200/70 bg-white shadow-[0_24px_60px_rgba(15,23,42,0.12)]">
                <img src={page.heroImage} alt={page.title} className="h-[360px] w-full object-cover" />
              </div>
      ...
      {sections.length ? (
        <Section className="bg-[#fbfaf7] border-t border-slate-200/70" pad="2.8rem">
          <div className="space-y-8">
            {sections.map((section, index) => (
              <div key={section.heading} className="grid gap-6 md:grid-cols-[1fr_1fr]">
                ...
                <div className="relative">
                  {section.image ? (
                    ...
                    <div className="h-56 rounded-[22px] border border-dashed border-slate-200 bg-white" />
                  )}
                </div>
              </div>
            ))}
